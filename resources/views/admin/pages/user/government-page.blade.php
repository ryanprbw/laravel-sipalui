            ...
            <div class="col-md-4">
                <x-card>
                    ...
                    <form action="{{ route('user-store') }}" method="post">
                        ...
                        <x-input type="hidden" name="level_id" value="2"/>
                        <div class="mt-3 mb-3 row">
                            <label>Pemerintahan</label>
                            <select class="select2 select-group" name="government_id">
                                <option selected disabled value="">.: Pilih Pemerintahan :.</option>
                                @foreach($governments as $government)
                                    <option data-name="{{$government->government_name}}"
                                            value="{{ $government->government_id }}">{{ $government->government_name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <x-input attr="readonly" name="name" title="Nama" placeholder="Nama"/>
